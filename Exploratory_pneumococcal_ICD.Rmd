...
p2 <- death_plot_fun(year,month, agec, J13) 
p2$plot1 + facet_wrap(agec~J13, scales='free', ncol=2)
```
...
mydata1 <- read.table('./Data/Wonder/Multiple Cause of Death, 2018-2020, Single Race_2022_08_25.txt', header=T, sep="\t")

mydata2 <- mydata1 %>%
  mutate(date= as.Date(paste0(substr(Month,1,3),substr(Year,1,4), '01'), '%b%Y%d'), month=lubridate::month(date),
         year=lubridate::year(date)) %>%
  filter(date<max(date)) #get rid of last month
